...
class DashboardController extends Controller
{
   ...
   public function ff_brands(Request $request)
    { 
        $team_id = ProfessionalDetails::select('team_id')->where('user_id','=',$request->ff_id)->get();
        $match = ['team_id' => $team_id[0]['team_id']]; 
        $brand_list = Brands::join('team_brands', 'brands.id', '=', 'team_brands.brand_id')
        ->where($match)->get();
        $returnData = [
