...
class GeneralFunctionsController extends Controller
{
    ...
    public  function quick_summary($user_type, $user_id, $start_date = '', $end_date = '', $ff_select = '')
    {
        ...
        if ($user_type == 'ff') {
            $ff_status = ['rx_entry.ff_id' => $user_id];
            $patients_ongoing_count = $patients_ongoing_count->where($ff_status);
            $patient_dropout_count = $patient_dropout_count->where($ff_status);
            $total_rxCount = $total_rxCount->where($ff_status);
            $dose_scheduled = $dose_scheduled->where($ff_status);
        }
    ...
    public  function dateRange($type)
    {
        if ($type == 'default') {
            ...
            $start_date = date('Y-m-d', strtotime(($year ) . '-'.($month - 5).'-01'));
            $end_date = date('Y-m-d', strtotime(($year + 1) . '-'.($month + 1).'-31'));
